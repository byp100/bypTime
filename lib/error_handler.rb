...
class ErrorHandler
  ...
  def self.handle_temp_token_errors(e, object)
    ...
    return object.render status: 400, json: { error_msg: error_msg }
  end
  ...
  def self.handle_invalid_request_errors(e, object, *param)
    ...
    if e.param.nil? || e.param.empty? || (!param.nil? && param.include?(e.param))
      ...
      return object.render status: 500, json: { error_msg: "Service has encountered an error. Please contact us." }
    else
  ...
  def self.handle_invalid_errors(e, object)
    if e.api_error_code == "invalid_state_for_request"
      ...
      puts e.json_obj
      return object.render status: 400, json: { error_msg: "Invalid state for this operation" }
    else
      return handle_invalid_request_errors(e, object)
    end
  end
  ...
  def self.handle_general_errors(e, object)
    ...
    return object.render :status => 500, :json => { error_msg: error_msg }
  end
